...
./restart_jenkinsdocker.py couchbasebuild/server-debian9-build:20190310 vulcan-debian9 5231 server.jenkins.couchbase.com
# Vulcan Ubuntu 16.04 slave - probably should use CV image as well,
# but don't want to rebuild that yet
./restart_jenkinsdocker.py couchbasebuild/server-ubuntu16-build:20190310 vulcan-ubuntu16.04 5239 server.jenkins.couchbase.com
# Primary zz-server-lightweight running on mega2 (same port as backup on mega3)
./restart_jenkinsdocker.py couchbasebuild/server-ubuntu16-build:20190310 zz-server-lightweight 5322 server.jenkins.couchbase.com
