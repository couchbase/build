...
# Test
CPDSN=${NODE_IP} ${PHP_DIR}/bin/php -d extension=phar.so -d extension=tokenizer.so -d extension=pcs.so -d extension=$(pwd)/modules/couchbase.so ${PHP_DIR}/phpunit.phar tests/
