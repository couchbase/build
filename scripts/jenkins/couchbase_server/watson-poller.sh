#!/bin/bash -ex
# Launched by "repo-watson" to see if a new build needs to be triggered.
#
...
diffchars=`repo diffmanifests ${WORKSPACE}/current.xml ${WORKSPACE}/build-team-manifests/watson.xml --raw | wc -c`
if [ "${diffchars}" = "0" ]
then
