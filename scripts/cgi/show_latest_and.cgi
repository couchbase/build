...
if ($outcome =~ 'good')
    {
    ($builder, $bldnum, $jobnum, $is_running, $bld_date, $bldstatus) = jenkinsReports::last_good_and_bld($platform, $branch, $edition);
    }
if ($outcome =~ 'done')
    {
    ($builder, $bldnum, $jobnum, $is_running, $bld_date, $bldstatus) = jenkinsReports::last_done_mobile_bld('and', $platform, $branch, $edition);
    }
...
if ($bldnum < 0)
    ...
    print_HTML_Page( jenkinsQuery::html_OK_link( $builder,  $jobnum,   $rev_numb, $bld_date),
                     jenkinsReports::link_to_package('and', $rev_numb, $platform, $edition ),
                     $builder,
                     $made_color );
    }
else
