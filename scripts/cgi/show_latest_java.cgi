...
# java build of master or release/1.0.0 branch.
#  
...
if ( $query->param('branch') && $query->param('edition') && $query->param('outcome') )
    ...
    {
    print_HTML_Page( buildbotQuery::html_ERROR_msg($usage), '&nbsp;', 'invalid call to show_latest_java.cgi', $err_color );
    exit;
    }
...
if ($outcome =~ 'good')
    {
    ($builder, $bldnum, $jobnum, $is_running, $bld_date, $bldstatus) = jenkinsReports::last_good_java_bld($platform, $branch, $edition);
    }
if ($outcome =~ 'done')
    {
