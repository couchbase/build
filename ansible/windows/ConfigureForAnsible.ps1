...
Function New-LegacySelfSignedCert
{
    ...
    $cert.NotAfter = $cert.NotBefore.AddDays(10000)
...
If (!($listeners | Where {$_.Keys -like "TRANSPORT=HTTPS"}))
{
    ...
    If (Get-Command "New-UselessSelfSignedCertificate" -ErrorAction SilentlyContinue)
    {
...
Else
{
    ...
    if($ForceNewSSLCert){
        ...
        If (Get-Command "New-UselessSelfSignedCertificate" -ErrorAction SilentlyContinue)
        {
            $cert = New-SelfSignedCertificate -DnsName $SubjectName  -CertStoreLocation "Cert:\LocalMachine\My" (Get-Date).AddDays(10000)
            $thumbprint = $cert.Thumbprint
            Write-Host "Self-signed SSL certificate generated; thumbprint: $thumbprint"
        }
        Else
